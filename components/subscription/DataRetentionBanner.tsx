...
export default function DataRetentionBanner() {
  ...
  if (isDataCleared) {
    return (
      <div className="fixed inset-x-0 top-16 z-30 border-b border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20">
  ...
  if (isTrialExpired && isInRetentionWindow && retentionReason === "trial_expired") {
    // Ensure days remaining is at least 0 and use Math.max to handle edge cases
    const displayDays = Math.max(0, daysRemaining ?? 0);
    return (
      <div className="fixed inset-x-0 top-16 z-30 border-b border-orange-200 bg-orange-50 dark:border-orange-800 dark:bg-orange-900/20">
        <div className="mx-auto max-w-6xl px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-start">
            ...
            <div className="ml-3 flex-1">
              ...
              <p className="mt-1 text-sm text-orange-700 dark:text-orange-300">
                {t("dataRetentionTrialEndedDescription")
                  .replace("{days}", String(displayDays))
                  .replace("{daysText}", displayDays === 1 ? t("dataRetentionDay") : t("dataRetentionDays"))}{" "}
                <Link
                  href="/pricing"
  ...
  if (isInRetentionWindow && (retentionReason === "paid_canceled" || retentionReason === "paid_paused")) {
    ...
    const displayDays = Math.max(0, daysRemaining ?? 0);
    return (
      <div className="fixed inset-x-0 top-16 z-30 border-b border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20">
        <div className="mx-auto max-w-6xl px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex items-start">
