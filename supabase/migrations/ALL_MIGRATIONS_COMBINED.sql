...
-- If you see 'already exists' notices, that's fine - those are already applied.
--
-- NOTE ON IDEMPOTENT MIGRATIONS:
-- Baseline migrations use idempotent patterns (CREATE IF NOT EXISTS, etc.) to
-- prevent errors when re-running. However, idempotent migrations only prevent
-- errors—they do NOT update existing objects if their structure has changed.
-- For structural changes, new migrations using explicit ALTER TABLE or
-- DROP/CREATE statements are required.
-- ============================================================================


-- ============================================================================
-- Migration: 20241124100000_agent_memory.sql
-- ============================================================================
...
-- ============================================================================
-- Migration: 20241125000000_rename_agent_types.sql
-- ============================================================================
...
-- ============================================================================
-- Migration: 20241126000000_gmail_connections.sql
-- ============================================================================
...
CREATE TRIGGER update_gmail_connections_updated_at
  BEFORE UPDATE ON public.gmail_connections FOR EACH ROW
  EXECUTE FUNCTION update_gmail_connections_updated_at();


-- ============================================================================
-- Migration: 20241126000001_calendar_connections.sql
-- ============================================================================
...
CREATE POLICY "Users can view own event notes"
  ON public.calendar_event_notes FOR SELECT
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can insert own event notes"
  ON public.calendar_event_notes FOR INSERT
  WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can update own event notes"
  ON public.calendar_event_notes FOR UPDATE
  USING (auth.uid() = user_id);
...
CREATE POLICY "Users can delete own event notes"
  ON public.calendar_event_notes FOR DELETE
  USING (auth.uid() = user_id);
...
CREATE TRIGGER update_calendar_connections_updated_at
  BEFORE UPDATE ON public.calendar_connections FOR EACH ROW
  EXECUTE FUNCTION update_calendar_connections_updated_at();

DROP TRIGGER IF EXISTS update_calendar_event_notes_updated_at ON public.calendar_event_notes;
CREATE TRIGGER update_calendar_event_notes_updated_at
  BEFORE UPDATE ON public.calendar_event_notes FOR EACH ROW
...
CREATE TABLE IF NOT EXISTS business_knowledge_chunks (
  ...
  -- Note: Changed from vector(1536) to TEXT to avoid requiring pgvector extension
  -- If pgvector is enabled, you can ALTER COLUMN to change this to vector(1536)
  embedding TEXT, -- OpenAI embedding dimension (stored as text/JSON, convert to vector if pgvector available)
...
-- Ensure primary_website_url column exists, then create index
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND column_name = 'primary_website_url'
  ) THEN
    ALTER TABLE business_profiles ADD COLUMN primary_website_url TEXT;
  END IF;
  
  -- Create index if column exists (should always be true after above)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'business_profiles' 
    ...
    CREATE INDEX IF NOT EXISTS idx_business_profiles_website 
      ON business_profiles(primary_website_url) 
      WHERE primary_website_url IS NOT NULL;
  END IF;
END $$;
...
-- GIN index for JSONB searches (conditional on column existence)
DO $$
BEGIN
  -- Ensure additional_urls column exists
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'business_profiles' 
    ...
    ALTER TABLE business_profiles ADD COLUMN additional_urls JSONB DEFAULT '[]'::jsonb;
  END IF;
  ...
  IF EXISTS (
    ...
    CREATE INDEX IF NOT EXISTS idx_business_profiles_additional_urls 
      ON public.business_profiles USING GIN(additional_urls);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE business_profiles ADD COLUMN preferences JSONB DEFAULT '{}'::jsonb;
  END IF;
  ...
  IF EXISTS (
    ...
    CREATE INDEX IF NOT EXISTS idx_business_profiles_preferences 
      ON public.business_profiles USING GIN(preferences);
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can view knowledge chunks for their business profile" ON public.business_knowledge_chunks;
...
DROP POLICY IF EXISTS "Users can insert knowledge chunks for their business profile" ON public.business_knowledge_chunks;
...
DROP POLICY IF EXISTS "Users can update knowledge chunks for their business profile" ON public.business_knowledge_chunks;
...
DROP POLICY IF EXISTS "Users can delete knowledge chunks for their business profile" ON public.business_knowledge_chunks;
...
-- Ensure watermark columns exist and add comments conditionally
DO $$
BEGIN
  -- Add image_watermark_enabled if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'business_profiles' 
    AND column_name = 'image_watermark_enabled'
  ) THEN
    ALTER TABLE business_profiles ADD COLUMN image_watermark_enabled BOOLEAN DEFAULT FALSE;
  END IF;
  
  -- Add image_watermark_text if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'business_profiles' 
    AND column_name = 'image_watermark_text'
  ) THEN
    ALTER TABLE business_profiles ADD COLUMN image_watermark_text TEXT;
  END IF;
  
  -- Add image_watermark_logo_url if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'business_profiles' 
    AND column_name = 'image_watermark_logo_url'
  ) THEN
    ALTER TABLE business_profiles ADD COLUMN image_watermark_logo_url TEXT;
  END IF;
  
  -- Add image_watermark_position if missing (with CHECK constraint)
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'business_profiles' 
    AND column_name = 'image_watermark_position'
  ) THEN
    ALTER TABLE business_profiles ADD COLUMN image_watermark_position TEXT 
      CHECK (image_watermark_position IN ('top_left', 'top_right', 'bottom_left', 'bottom_right', 'center', 'top_center', 'bottom_center'));
  END IF;
...
DROP POLICY IF EXISTS "Users can view their own call campaigns" ON public.call_campaigns;
CREATE POLICY "Users can view their own call campaigns"
  ON public.call_campaigns FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own call campaigns" ON public.call_campaigns;
CREATE POLICY "Users can insert their own call campaigns"
  ON public.call_campaigns FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own call campaigns" ON public.call_campaigns;
CREATE POLICY "Users can update their own call campaigns"
  ON public.call_campaigns FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own call campaigns" ON public.call_campaigns;
...
DROP POLICY IF EXISTS "Users can view targets for their campaigns" ON public.call_campaign_targets;
...
DROP POLICY IF EXISTS "Users can insert targets for their campaigns" ON public.call_campaign_targets;
...
DROP POLICY IF EXISTS "Users can update targets for their campaigns" ON public.call_campaign_targets;
...
DROP POLICY IF EXISTS "Users can delete targets for their campaigns" ON public.call_campaign_targets;
...
DROP POLICY IF EXISTS "Users can view their own knowledge gaps" ON public.agent_knowledge_gaps;
CREATE POLICY "Users can view their own knowledge gaps"
  ON public.agent_knowledge_gaps FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own knowledge gaps" ON public.agent_knowledge_gaps;
CREATE POLICY "Users can insert their own knowledge gaps"
  ON public.agent_knowledge_gaps FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own knowledge gaps" ON public.agent_knowledge_gaps;
CREATE POLICY "Users can update their own knowledge gaps"
  ON public.agent_knowledge_gaps FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own knowledge gaps" ON public.agent_knowledge_gaps;
...
DROP POLICY IF EXISTS "Users can view their own contact profiles" ON public.contact_profiles;
DROP POLICY IF EXISTS "Users can view their own contact profiles" ON public.contact_profiles;
CREATE POLICY "Users can view their own contact profiles"
  ON public.contact_profiles FOR SELECT
  USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can insert their own contact profiles" ON public.contact_profiles;
DROP POLICY IF EXISTS "Users can insert their own contact profiles" ON public.contact_profiles;
CREATE POLICY "Users can insert their own contact profiles"
  ON public.contact_profiles FOR INSERT
  WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can update their own contact profiles" ON public.contact_profiles;
DROP POLICY IF EXISTS "Users can update their own contact profiles" ON public.contact_profiles;
...
DROP POLICY IF EXISTS "Users can delete their own contact profiles" ON public.contact_profiles;
DROP POLICY IF EXISTS "Users can delete their own contact profiles" ON public.contact_profiles;
...
-- Ensure all voicemail-related columns exist before using them in triggers/functions
DO $$
BEGIN
  -- Add voicemail_enabled if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'user_phone_numbers' 
    AND column_name = 'voicemail_enabled'
  ) THEN
    ALTER TABLE user_phone_numbers ADD COLUMN voicemail_enabled BOOLEAN NOT NULL DEFAULT false;
  END IF;
  
  -- Add voicemail_mode if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'user_phone_numbers' 
    ...
    ALTER TABLE user_phone_numbers ADD COLUMN voicemail_mode TEXT NOT NULL DEFAULT 'none' 
      CHECK (voicemail_mode IN ('none', 'voicemail_only', 'receptionist'));
  END IF;
  
  -- Add forwarding_enabled if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'user_phone_numbers' 
    ...
    ALTER TABLE user_phone_numbers ADD COLUMN forwarding_enabled BOOLEAN NOT NULL DEFAULT false;
  END IF;
  
  -- Add forwarding_confirmed if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'user_phone_numbers' 
    AND column_name = 'forwarding_confirmed'
  ) THEN
    ALTER TABLE user_phone_numbers ADD COLUMN forwarding_confirmed BOOLEAN NOT NULL DEFAULT false;
  END IF;
  
  -- Add external_phone_number if missing
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' 
    AND table_name = 'user_phone_numbers' 
    AND column_name = 'external_phone_number'
  ) THEN
    ALTER TABLE user_phone_numbers ADD COLUMN external_phone_number TEXT;
  END IF;
END $$;

-- Add comments (conditional on column existence)
...
-- Add comments conditionally (only if columns exist)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Note: This function assumes external_phone_number column exists (ensured above)
...
BEGIN
  ...
  IF OLD.is_active = true AND NEW.is_active = false THEN
    ...
    -- Set external_phone_number to NULL (column is ensured to exist above)
...
DROP TRIGGER IF EXISTS trigger_disable_voicemail_on_release ON public.user_phone_numbers;
...
DROP TRIGGER IF EXISTS trigger_update_user_phone_numbers_updated_at ON public.user_phone_numbers;
...
-- Ensure all voicemail-related columns exist (in case this migration runs before the voicemail one)
DO $$
BEGIN
  -- Add voicemail_enabled if missing
  IF NOT EXISTS (
    ...
    ALTER TABLE user_phone_numbers ADD COLUMN voicemail_enabled BOOLEAN DEFAULT false;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_phone_numbers ADD COLUMN voicemail_mode TEXT DEFAULT 'none' 
      CHECK (voicemail_mode IN ('none', 'voicemail_only', 'receptionist'));
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_phone_numbers ADD COLUMN forwarding_enabled BOOLEAN DEFAULT false;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_phone_numbers ADD COLUMN forwarding_confirmed BOOLEAN DEFAULT false;
  END IF;
...
-- Comments for forwarding columns are now in the DO block above
...
DROP POLICY IF EXISTS "Users can view their own briefs" ON public.insight_briefs;
CREATE POLICY "Users can view their own briefs"
  ON public.insight_briefs FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own briefs" ON public.insight_briefs;
CREATE POLICY "Users can insert their own briefs"
  ON public.insight_briefs FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own briefs" ON public.insight_briefs;
...
DROP POLICY IF EXISTS "Users can view seats in their workspace" ON public.workspace_seats;
...
DROP POLICY IF EXISTS "Workspace owners can manage seats" ON public.workspace_seats;
...
DROP POLICY IF EXISTS "Users can view invites for their workspace" ON public.workspace_invites;
...
DROP POLICY IF EXISTS "Workspace owners can manage invites" ON public.workspace_invites;
...
DROP POLICY IF EXISTS "Users can insert own emails" ON public.email_queue;
CREATE POLICY "Users can insert own emails"
  ON public.email_queue FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Create policy: Users can update their own emails
DROP POLICY IF EXISTS "Users can update own emails" ON public.email_queue;
CREATE POLICY "Users can update own emails"
  ON public.email_queue FOR UPDATE
  USING (auth.uid() = user_id);

-- Create policy: Users can delete their own emails
DROP POLICY IF EXISTS "Users can delete own emails" ON public.email_queue;
